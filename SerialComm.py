...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.comboBox1.currentIndexChanged.connect(self.checkReadyToStart)
...
#        thread=ThreadClass()
        ...
        self.cancelButtonClick(self) # for init
    ...
    def file_read(self):
        ...
        for line in File:
            ...
            if((line.find('user = '))==0):
                ...
                user = line
                print str(user)
                USER = user[7:(len(user)-1)].strip(' \t\n\r')

            if((line.find('password = '))==0):
                global PASSWORD
                password = line
                print str(password)
                PASSWORD = password[11:(len(password))].strip(' \t\n\r')
...
#        ValueKG=""
    ...
    def selectionchange(self):
        ...
        if (ingrNum<len(self.ingredientList)):
            ...
            #collapse bulk ingridients
            new_batch_res = []
            self.weightTotal = 0
            for rec in self.batchRes:
                if len(new_batch_res) ==0 or rec['ingredient_id'] !=  new_batch_res[-1]['ingredient_id']:
                   new_batch_res.append(rec)
                else:
                    new_batch_res[-1]['weighed'] += rec['weighed']
                self.weightTotal += rec['weighed']
            self.batchRes = new_batch_res
            ...
            for r in self.batchRes:
                curs.execute("insert into batch_detail (id, batch_id, ingredient_id, weighed) values (0, %s, %s, %s)",  [self.curBatchId,  r['ingredient_id'],  r['weighed']])
            curs.execute("update batch b set status = 2, timestamp = now() where id = %s",  [self.curBatchId])
    ...
    def CHECK_DATA(self,value):
        if(self.combineDATA):
            X1=single_ingredient_WT +  single_ingredient_WT * single_ingredient_TL
            X2=single_ingredient_WT -  single_ingredient_WT * single_ingredient_TL
...
rent Weight Value.
    ...
    def Call(self,j):
        ...
        if  self.ingredientList[j]['bulk_step']:
            self.bulk_label.setText('Bulk step {0} for ingridient {1}'.format(self.ingredientList[j]['bulk_step'], self.ingredientList[j]['ingredient_name']))
        else:
    ...
    def text_FILE(self,i):
        ...
        for reading in curs.fetchall():
            #print str(reading[0])
            if reading[4] == None or reading[4] == 0 or reading[4] >= reading[3]:
                self.ingredientList.append({'ingredient_id':reading[0],  'ingredient_name':reading[1],  'tolerance': reading[2], 'requested':reading[3], 'bulk_step': 1 if reading[4] else None })
            else:
                remaining_requested = reading[3]
                step = 0
                part_weight = reading[4]
                while remaining_requested > 0:
                    step +=1
                    if remaining_requested > reading[4]:
                        remaining_requested -= reading[4]
                    else:
                        part_weight = remaining_requested
                        remaining_requested = 0
                    self.ingredientList.append({'ingredient_id':reading[0],  'ingredient_name':reading[1],  'tolerance': reading[2], 'requested':part_weight, 'bulk_step': step})
    ...
    def cancelButtonClick(self,  b=None):
        ...
        self.cancelButton.hide()
        self.butchNP = ''
        self.batch_lineEdit.show()
        self.batch_lineEdit.setText('')
